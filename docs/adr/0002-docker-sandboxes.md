...
(`mcp/sandbox_tools.rs`): the 25 desktop and system tools minus `list_keybinds`, with the
...
gateway, Docker Desktop's host addresses, and private or link-local networks
(including LAN addresses on the host). A start that cannot be
