...
left enabled. On Linux, if Tailscale requires operator access, Conduit asks for
your password once and grants it through sudo before enabling Funnel. The
password is sent to sudo's stdin and is not saved.
...
Agents get the same desktop and system tools under the same names (minus
`list_keybinds`; browser-specific tools target only the host browser), acting
