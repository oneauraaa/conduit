...
use crate::mcp::tools::{
    ...
    ReadScreenArgs, ScreenshotArgs, ScrollArgs, ShellArgs, TextArgs, TypeArgs, WaitArgs,
...
impl SandboxConduit {
    ...
    #[tool(
        description = "Run a command in the sandbox with bash (as the `conduit` user, in its \
    ...
    async fn run_shell(
        ...
        gate::run_sandbox(
            ...
            || async {
                ...
                let command = shell_argv(seconds, &marker, &args.command);
...
fn shell_argv(seconds: u64, marker: &str, command: &str) -> Vec<String> {
    vec![
        "timeout".into(),
        "-k".into(),
        "5".into(),
...
pub(crate) fn instructions(spec: &SandboxSpec) -> String {
    ...
    format!(
        "conduit gives you a sandbox: a {os} desktop (XFCE) named \"{name}\", running in Docker \
         ...
         run_shell runs bash as the user `conduit` in /home/conduit, which persists across \
...
mod tests {
    ...
    fn shell_commands_are_bounded_and_tagged_without_touching_dollar_zero() {
        let argv = shell_argv(30, "conduit-call-abc", "echo hi");
        assert_eq!(&argv[..6], ["timeout", "-k", "5", "30", "bash", "-lc"]);
